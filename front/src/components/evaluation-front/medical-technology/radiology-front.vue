    ...
    <div class="right">
      <table id="table">
        ...
        <tr>
          <td>质控资料上传</td>
          <!--        抛出信息-->
          <td>{{ clinicData && clinicData.mqDataMessage1 }}</td>
          <td></td>
        </tr>
        <tr>
          <td>管理工具使用</td>
          <td>{{ clinicData && clinicData.mqDataMessage2 }}</td>
          <td></td>
        </tr>
        <tr>
          <td>持续改进效果评价</td>
          <td>{{ clinicData && clinicData.mqDataMessage3 }}</td>
          <td></td>
        </tr>
        ...
        <tr>
          <td>注</td>
          <td>
            1、以上考核中各项目解释权分别为：门诊考核归门诊部，合理用药考核归药学部，用血考核归输血科，不良事件上报及患者权益归医患关系办，公卫、预防保健、传染病质控归公共卫生科，其余项目归质控科。<br>
            2、部分考核结果已提前公示通知各科负责人并告知最后申诉时间，过期不再接受申诉。<br>
            3、未提前公示的内容在本质控单发放后次日下午17:00结束申诉。
          </td>
          <td></td>
        </tr>
...
export default {
  ...
  setup() {
    ...
    const exportTable = () => {
      ...
      exportToExcel(fscore.value, finalAppeal.value,table);
    };
