<template>
  <div class="container">
    <div class="left">
      ...
      <div class="form-group">
        <button @click="saveFinalScore">保存最终扣分</button>
      </div>
      <div class="form-group">
    ...
    <div id="tables">
      ...
      <table id="table">
        ...
        <tr>
          ...
          (clinicData.difficultCaseData[0] &&
           ...
           clinicData.difficultCaseData[0].participation == null &&
            ...
            主持人是否签字: {{ clinicData && clinicData.difficultCaseData[0] && clinicData.difficultCaseData[0].hostSigned}}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;是否签字潦草: {{ clinicData && clinicData.difficultCaseData[0] && clinicData.difficultCaseData[0].scrawledSignature}}
          </span>
        ...
        <tr>
          <td>急诊病历书写</td>
          <td>
            <template v-if="medicalData && medicalData.emergencyMedicalRecordsData1">
              <div v-for="(item, index) in medicalData.emergencyMedicalRecordsData1" :key="index">
                {{ item.doctorName }}，{{ item.outpatientNumber }}，{{ item.summary }}，{{ item.totalDeductionPoints }}
              </div>
            </template>
          </td>
          <td>{{ medicalData && medicalData.emergencyMedicalRecordsData2[0] && medicalData.emergencyMedicalRecordsData2[0].totalPointsDeducted }}</td>
        </tr>

        <tr>
          <td>院前及院内急救病历</td>
          <td>
            <template v-if="medicalData && medicalData.emergencyMedicalRecordsData3">
              <div v-for="(item, index) in medicalData.emergencyMedicalRecordsData3" :key="index">
                {{ item.doctorName }}，{{ item.outpatientNumber }}，{{ item.summary }}，{{ item.totalDeductionPoints }}
              </div>
            </template>
          </td>
          <td>{{ medicalData && medicalData.emergencyMedicalRecordsData4[0] && medicalData.emergencyMedicalRecordsData4[0].totalPointsDeducted }}</td>
        </tr>

        <tr>
          <td>留观病历</td>
          <td>
            <template v-if="medicalData && medicalData.emergencyMedicalRecordsData5">
              <div v-for="(item, index) in medicalData.emergencyMedicalRecordsData5" :key="index">
                {{ item.doctorName }}，{{ item.outpatientNumber }}，{{ item.summary }}，{{ item.totalDeductionPoints }}
              </div>
            </template>
          </td>
          <td>{{ medicalData && medicalData.emergencyMedicalRecordsData6[0] && medicalData.emergencyMedicalRecordsData6[0].totalPointsDeducted }}</td>
        </tr>
...
import evaluateScore from '@/utils/evaluateScore2.js';
...
export default {
  ...
  setup() {
    ...
    const medicalData = ref(null); // Declare a reactive variable to store the retrieved medical data

    const fscore = reactive({ value: null });  // 使用 ref 在 setup 中定义 fscore
    const escore2 = ref(0); // 声明一个新的响应式变量来存储得分
    ...
    const submitForm = () => {
      ...
      axios.post(`http://localhost:8998/api/export/form/clinic?officeName=${form.value.select}&userTime=${form.value.date}`)
          .then(response => {
            ...
            escore2.value = evaluateScore(clinicData.value, medicalData.value);// 计算得分并存到响应式变量中
      ...
      //查询急诊病历
      axios.post(`http://localhost:8998/api/export/form/medical?officeName=${form.value.select}&userTime=${form.value.date}`)
          .then(response => {
            // Handle successful response
            console.log(response.data);
            medicalData.value = response.data; // Store the retrieved data in the reactive variable
          })
          .catch(error => {
            // Handle error
            console.error(error.response.data);
    ...
    const exportTable = () => {
      ...
      const fileName = selectInput.value;  // 获取select元素的值
