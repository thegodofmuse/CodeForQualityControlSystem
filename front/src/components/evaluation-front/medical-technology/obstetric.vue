<template>
  <div class="container">
    ...
    <div class="right">
      <div id="tables">
        ...
        <table id="table">
          ...
          <tr>
            <td>产科特别质控</td>
            <td>
              <template v-if="clinicData && clinicData.obstetricSpecialData">
                <div v-for="(item, index) in clinicData.obstetricSpecialData" :key="index">
                  纯母乳喂养率:{{ toPercentage(item.exclusiveBreastfeeding)}}<br>
                  非医学指征剖宫产率：{{ toPercentage(item.abnormalCesarean) }}<br>
                  剖宫产率：{{ toPercentage(item.cesarean)}}<br>
                  阴道分娩并发症:{{ toPercentage(item.complicationsOfVaginalDelivery)}}<br>
                </div>
              </template>
            </td>
            <td></td>
      ...
      最终申诉请填到这里来：<textarea v-model="finalAppeal" class="final-appeal-input"></textarea>
    </div>
  </div>
